/*Write a program to take an array arr[] of integers as input, the task is to find the next
...
Next greater element of an element in the array is the nearest element on the right which
 is greater than the current element. If there does not exist greater of current element,
